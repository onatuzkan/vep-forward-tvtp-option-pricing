...
def residual_moments(
    ...
    for j in range(n - 1):
        ...
        k1 = deriv(t[j], state)
        k2 = deriv(t[j] + 0.5 * h, state + 0.5 * h * k1)
        k3 = deriv(t[j] + 0.5 * h, state + 0.5 * h * k2)
        k4 = deriv(t[j] + h, state + h * k3)
...
def simulate_forward_centered(
    ...
    """Exact-in-distribution Monte Carlo cross-check of the residual PDE.

    Per step the regime jumps with the exact frozen-generator probability and
    the residual advances with the exact OU transition.  Reports the random
