...
The module runs without market data: `calibrate` simply requires a list of
ForwardQuote objects, and `synthetic_self_test` verifies that calibrating to
model-generated forwards recovers (approximately) zero premia.  Forward prices
...
def synthetic_self_test(
    inputs: MarkovInputs,
    scenario: ScenarioPath,
    maturities_hours: Sequence[float] = (24.0, 48.0),
    grid_settings: Optional[GridSettings] = None,
    true_a: Tuple[float, float] = (-0.004, -0.004),
) -> CalibrationResult:
    """Round-trip test that actually exercises the optimizer.

    1. Generate target forwards under a KNOWN nonzero drift premium
       a_i = true_a (default -0.004/h, the magnitude relevant for flattening
       the near-unit-root forward drift).
    2. Calibrate starting from a = 0 and check the premium is recovered.

    A degenerate variant (calibrating to the unadjusted model's own forwards
    from x0 = 0) converges trivially at the start point and tests nothing.
    """
    gs = grid_settings or GridSettings()
    adj_true = baseline_q1(drift_shift_per_hour=true_a)
    quotes = []
    for h in maturities_hours:
        T = inputs.valuation_utc + pd.Timedelta(hours=float(h))
        f0 = model_forward_price(inputs, scenario, T, adjustment=adj_true,
                                 grid_settings=gs)
        quotes.append(ForwardQuote(maturity_utc=T, price=f0))
    res = calibrate(inputs, scenario, quotes, free=("a0", "a1"),
                    lambda_reg=1e-10, grid_settings=gs)
    err = np.array([res.theta_hat["a0"] - true_a[0],
                    res.theta_hat["a1"] - true_a[1]])
    logger.info("self-test: true a=%s, recovered=%s, abs err=%s",
                true_a, res.theta_hat, np.abs(err))
    return res
